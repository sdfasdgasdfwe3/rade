...
cleanup() {
    echo "Скрипт завершается. Выполняем очистку..."
    # Добавьте сюда команды, которые нужно выполнить при завершении
    # Например, удаление временных файлов, остановка процессов и т.д.
}
