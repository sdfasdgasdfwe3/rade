...
sleep 2  # даём время для клонирования

# Проверяем, создалась ли папка
if [ ! -d "~/rade" ]; then
    echo "Ошибка: папка 'rade' не была создана. Проверьте, успешно ли клонируется репозиторий."
    exit 1
fi

echo "-----------------------------------------"
echo "Переходим в директорию репозитория..."
