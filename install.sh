...
if [ -f "auth_token.txt" ]; then
  ...
  echo "Авторизация не выполнена. Переходим к запуску бота без запроса данных."
  # Просто создаем пустой файл auth_token.txt, чтобы пропустить дальнейшую авторизацию
  touch auth_token.txt
fi

echo "-----------------------------------------"
echo "Запускаем бота..."
# Просто запускаем бота без фона
python bot.py
