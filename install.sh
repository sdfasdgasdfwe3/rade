...
# Путь для файла сессии в домашней директории
...
if [ -f "$SESSION_FILE" ]; then
    echo "Сессия найдена. Проверяем, не пустой ли файл..."
    if [ ! -s "$SESSION_FILE" ]; then
        echo "Файл сессии пустой. Удаляем его..."
        rm "$SESSION_FILE"  # Удаляем пустой файл сессии
    else
        echo "Сессия найдена и не пустая. Создаём резервную копию..."
...
# Проверка, был ли успешно клонирован репозиторий
if [ ! -d "rade" ]; then
    echo "Ошибка: репозиторий не был клонирован. Директория 'rade' не найдена."
    exit 1
else
...
# Проверка, что мы действительно в директории 'rade'
if [ "$(pwd)" != "$HOME/rade" ]; then
    echo "Ошибка: не удалось перейти в директорию 'rade'. Текущая директория: $(pwd)"
    exit 1
fi
...
echo "-----------------------------------------"
echo "Проверяем наличие резервной копии сессии..."

# Проверка резервной копии сессии
if [ -f "${SESSION_FILE}.backup" ]; then
    echo "Резервная копия найдена. Восстанавливаем сессию..."
