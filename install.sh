...
echo "-----------------------------------------"
echo "Проверяем, существует ли сессия..."
SESSION_FILE="session_$(grep -oP '(?<=\bPHONE_NUMBER": ")[^"]+' config.json).session"
if [ -f "$SESSION_FILE" ]; then
  echo "Сессия уже существует. Бот не будет перезапускать сессию."
else
  echo "Сессия не найдена. Создаём новую сессию."
fi
