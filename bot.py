...
def check_for_updates():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Извлечение версии из скрипта (при условии, что SCRIPT_VERSION объявлена в виде: SCRIPT_VERSION = "..." )
...
async def animated_typing(event):
    """Команда для анимации печати текста (/p <текст>)."""
    try:
...
async def update_script(event):
    """Команда для обновления скрипта с GitHub и автоматического перезапуска."""
    try:
        response = requests.get(GITHUB_RAW_URL)
        if response.status_code == 200:
            current_file = os.path.abspath(__file__)
...
async def main():
    ...
    print("Используйте /p <текст> для анимации печати и /update для обновления скрипта.")
