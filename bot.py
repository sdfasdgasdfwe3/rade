...
def check_input():
    while True:
        ...
        if user_input.lower() == "magic":  # Проверяем на слово "Magic"
            print("Вы ввели кодовое слово 'Magic', запускаем второй скрипт...")
            run_script()  # Запуск второго скрипта
            break  # Прерывание, если второй скрипт был запущен
        time.sleep(1)  # Задержка между проверками, чтобы снизить нагрузку на процессор

# Функция для выполнения других задач в основном потоке
def main():
    while True:
        # Ваши другие задачи, которые не блокируют выполнение программы
        print("Программа выполняет другие действия...")
        time.sleep(5)

# Создаем поток для проверки ввода
input_thread = threading.Thread(target=check_input)
input_thread.daemon = True  # Делает поток фоновым
input_thread.start()
